...
{
	if(IsValid(TargetCharacter) && DamageComponent && IsLocalController())
	{
		UDamageTextComponent* DamageText = NewObject<UDamageTextComponent>(TargetCharacter,DamageComponent);
...
void AAuraPlayerController::CursorTrace()
{
	...
	const ECollisionChannel TraceChannel = IsValid(MagicCircle) ? ECC_ExludePlayers : ECC_Visibility;
...
{
	if (InActor && InActor->Implements<UHighlightInterface>())
	{
...
void AAuraPlayerController::AbilityInputTagPressed(FGameplayTag InputTag)
{
	...
	if (InputTag.MatchesTagExact(FAuraGamePlayTags::Get().InputTag_LMB) )
	{
		if (ThisActor)
		{
			TargetingStatus = ThisActor->Implements<UIEnemyInterface>() ? TargetingEnemy : TargetingActor;
			bAutoRunning = false;
		}
		else
		{
			TargetingStatus = TargetingNone;
		}
